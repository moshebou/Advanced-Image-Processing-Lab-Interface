...
%% Reference:
% 
function ApertureCorrection = ApertureCorrection_mb( handles )
	...
	interface_params(k).callback = @(a,b)run_process_image(a);
 	...
 	k=k+1;              
	interface_params = SetSliderParams('Set frequency domain threshold', 0.1, 0, ApertureCorrection.ThrF, 0.0025, @(a,b)SetParam(a,b,k,handles.current_experiment_name, 'ThrF',@update_sliders), interface_params, k);
	...
	k=k+1;
	interface_params = SetSliderParams('Additive noise standard deviation', 50, 0, ApertureCorrection.std, 1, @(a,b)SetParam(a,b,k,handles.current_experiment_name, 'std',@update_sliders), interface_params, k);
    %%tive noise

	k=k+1;      
	interface_params = SetSliderParams('Set bluring filter PSF size', 21, 1, ApertureCorrection.filter_size, 1, @(a,b)SetParam(a,b,k,handles.current_experiment_name, 'filter_size',@update_sliders), interface_params, k);
