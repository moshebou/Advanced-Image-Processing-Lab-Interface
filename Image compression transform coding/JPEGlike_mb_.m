...
if Encode
   ...
   if Speed
      ...
      % save HL 
      for j=1:16
         for (i=4:-1:1); [y, Byte, BitPos] = PutBit(bitget(HL(j),i), y, Byte, BitPos); end;
      end
      HK=HuffCode_mb(HL);
      ...
      for l=1:L
         ...
         for k=1:HL(n)
            [y, Byte, BitPos] = PutBit(HK(n,k), y, Byte, BitPos);
         end
         if DCsym(l)        % extra bits
            if (DC(l)>0)    % the sign '1'='+', '0'='-'
               [y, Byte, BitPos] = PutBit(1, y, Byte, BitPos);
               n=DC(l)-2^(DCsym(l)-1);
            else; 
                [y, Byte, BitPos] = PutBit(0, y, Byte, BitPos); 
               n=DC(l)+2^DCsym(l)-1;
            end;  
            % if Debug; disp(['   n=',int2str(n)]); end;
            for k=(DCsym(l)-1):-1:1
               [y, Byte, BitPos] = PutBit(bitget(n,k), y, Byte, BitPos);
            end
         end
      end
   end
   ...
   if Speed
      ...
      % save HL 
      for i=1:16
         for (j=8:-1:1); [y, Byte, BitPos] = PutBit(bitget(Hii(i+1),j), y, Byte, BitPos); end;
      end
      for i=1:16
         I=find(HL==i);        % find symbols with codeword length i
         I=I-1;                % symbols in range 0:255
         for k=1:Hii(i+1)
            for (j=8:-1:1); [y, Byte, BitPos] = PutBit(bitget(I(k),j), y, Byte, BitPos); end;
         end
      end
      HK=HuffCode_mb(HL);
...
if Decode
   ...
   HL=zeros(1,16);
   for j=1:16
      for (i=1:4); HL(j)=HL(j)*2+Get_Bit; end;
   end
   ...
   while l<L
      ...
      if Htree(pos,1)           % we have arrived at a leaf
         ...
         if DCsym(l)
            DC(l)=2^(DCsym(l)-1);
            if (~(Get_Bit)); DC(l)=1-2*DC(l); end;
            if (DCsym(l)>1)
               n=0;
               for i=1:(DCsym(l)-1); n=n*2+Get_Bit; end;
               DC(l)=DC(l)+n;
            end
         end
         if Debug
   ...
   % disp([int2str(Byte),'  ',int2str(BitPos)]);
   for j=1:16
      for (i=1:8); Hii(j)=Hii(j)*2+Get_Bit; end;
   end
   % disp(Hii);
   HL=zeros(1,256);
   for j=1:16
   ...
   while ~Finished
      ...
      if Htree(pos,1)           % we have arrived at a leaf
         ...
         if SS
            AC=2^(SS-1);
            if (~(Get_Bit)); AC=1-2*AC; end;
            if (SS>1)
               k=0;
               for i=1:(SS-1); k=k*2+Get_Bit; end;
               AC=AC+k;
            end
         end
         if Debug
...
function [y, Byte, BitPos] = PutBit(Bit, y, Byte, BitPos)
...
function [Bit , y, Byte, BitPos]=GetBit(y,Byte,BitPos)
%global y Byte BitPos
BitPos=BitPos-1;
if (~BitPos); Byte=Byte+1; BitPos=8; end; 
...
global y Byte BitPos
[Bit , y, Byte, BitPos]=GetBit(y,Byte,BitPos);
return;
% test of function
