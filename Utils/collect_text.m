function collected_text = collect_text(head_folder, file_type, start_str, end_str, rmv_str)
    collected_text = collect_text_recursive(head_folder, file_type, start_str, end_str, rmv_str);
end

function text = collect_text_recursive(head_folder, file_type, start_str, end_str, rmv_str)
    list = dir (head_folder);
    text = [];
    for i=1:length(list)
        if ( list(i).isdir && ~(list(i).name(end) == '.'))
            tmp =  collect_text_recursive([head_folder list(i).name '\'], file_type, start_str, end_str, rmv_str);
            if ( isempty(text) ) 
                text = tmp; 
            elseif ( ~isempty(tmp))
