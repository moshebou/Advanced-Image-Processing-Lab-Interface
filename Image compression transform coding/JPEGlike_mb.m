function varargout = JPEGlike_mb(arg1, arg2, arg3)
...
% Function needs following m-files: HuffLen_mb, HuffCode_mb, HuffTree_mb
...
%              BitPos=BitPos-1;
%             if (~BitPos); Byte=Byte+1; BitPos=8; end; 
%             y(Byte) = bitset(y(Byte),BitPos,bitget(HL(j),i));
%          end
%       end
%       y_tmp=zeros(ceil(bits/8),1);
      BitPos = 1;
      Byte = 8;
      C = reshape(HL, 2, [])'; 
      y(1:size(C,1)) = C(:,1)*2^4+C(:,2);
      [HK HK_val]=HuffCode_mb(HL);
      
      %%
% % % % % % % %       n = DCsym(:) +1;
% % % % % % % %       values = bitshift(uint32(HK_val(n)), -8+HL(n)');
% % % % % % % %       values = (double(values)*2 + double(DC(:)>0)).*(2.^(DCsym(:)-1))+ ...
% % % % % % % %           (DC(:)>0).*bitand((DC(:)>0).*(DC(:)-2.^(DCsym(:)-1)),(DC(:)>0).*(2.^(DCsym(:)-1) -1) ) +   ...
% % % % % % % %           (DC(:)<0).*bitand((DC(:)<0).*(DC(:)+2.^DCsym(:)-1),(DC(:)<0).*(2.^(DCsym(:)-1) -1) ) ;
% % % % % % % %       bit_used = HL(n)' + DCsym(:);
% % % % % % % %       res = zeros(1, ceil(sum(bit_used)/8));
% % % % % % % %       l=1;
% % % % % % % %       while sum(mod(bit_used(l:end),8))
% % % % % % % %           if ( bit_used(l) > 8 )
% % % % % % % %               move_bits = bitand(values(l:end), max(0, 2.^(bit_used(l:end)- 8) - 1));
% % % % % % % %               values = [0 ; move_bits(l:end-1).*2.^bit_used(l+1:end)] + bitshift(values(l:end), min(0,8-bit_used(l:end)));
% % % % % % % %               bit_used = [0; max(0, bit_used(l:end-1)-8)]  + bit_used - max(0, bit_used - 8);
% % % % % % % %               l=l+1;
% % % % % % % %               values = values(bit_used~=0);
% % % % % % % %               bit_used = bit_used(bit_used~=0);
% % % % % % % %           end
% % % % % % % %           if ( bit_used(l) < 8 )
% % % % % % % %               move_bits = bitand(uint32(values), max(0, 2.^(8-bit_used) - 1));
% % % % % % % %               values = [0 move_bits(l+1:end)*2^8] + bitshift(uint32(values), 8-bit_used);
% % % % % % % %               bit_used(l+1:end) = (8-bit_used(l:end-1)) + bit_used(l+1:end);
% % % % % % % %               l=l+1;
% % % % % % % %               values = values(bit_used~=0);
% % % % % % % %           end
% % % % % % % %            if ( bit_used(l) == 8 )
...
% % % % % % % %       end
      ...
      %y_k(Byte+1:length(HK_val(n))+Byte) = HK_val(n);
      ...
      for l=1:L
         ...
         for k=1:HL(n)
           BitPos=BitPos-1;
            if (~BitPos); Byte=Byte+1; BitPos=8; end; 
            y(Byte) = bitset(y(Byte),BitPos,HK(n,k));
         end
         ...
         if DCsym(l)        % extra bits
            if (DC(l)>0)    % the sign '1'='+', '0'='-'
               BitPos=BitPos-1;
                if (~BitPos); Byte=Byte+1; BitPos=8; end; 
                y(Byte) = bitset(y(Byte),BitPos,1); 
               n=DC(l)-2^(DCsym(l)-1);
            else
               BitPos=BitPos-1;
            if (~BitPos); Byte=Byte+1; BitPos=8; end; 
            y(Byte) = bitset(y(Byte),BitPos,0); 
               n=DC(l)+2^DCsym(l)-1;
            end;  
            ...
            for k=(DCsym(l)-1):-1:1
                ...
                BitPos=BitPos-1;
                if (~BitPos); Byte=Byte+1; BitPos=8; end; 
                y(Byte) = bitset(y(Byte),BitPos,bitget(n,k));
            end
         end
      end
   end
   ...
   if Speed
      ...
      for i=1:16
         for (j=8:-1:1)               
                BitPos=BitPos-1;
                if (~BitPos); Byte=Byte+1; BitPos=8; end; 
                y(Byte) = bitset(y(Byte),BitPos,bitget(Hii(i+1),j)); 
         end
      end
      for i=1:16
         ...
         for k=1:Hii(i+1)
            for (j=8:-1:1) 
                BitPos=BitPos-1;
                if (~BitPos); Byte=Byte+1; BitPos=8; end; 
                y(Byte) = bitset(y(Byte),BitPos,bitget(I(k),j)); 
            end;
         end
      end
      HK=HuffCode_mb(HL);
