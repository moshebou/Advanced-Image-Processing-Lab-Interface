...
function UnsharpMasking = UnsharpMasking_mb(handles)
	...
	UnsharpMasking = DeployAxes( handles.figure1, ...
	[axes_hor, ...
	axes_ver], ...
	bottom, ...
	left, ...
	...
	UnsharpMasking.im = HandleFileList('load' , HandleFileList('get' , handles.image_index)); 

	%init params
	UnsharpMasking.WdSzX = 3;
	UnsharpMasking.WdSzY = 3;
	UnsharpMasking.evpos = 5;
	UnsharpMasking.evneg = 5;
	UnsharpMasking.erpos = 5;
	UnsharpMasking.erneg = 5;
	...
	k=k+1;
	interface_params =  SetSliderParams('Set positive EV offset', 41, 1, UnsharpMasking.evpos, 1, @(a,b)SetParam(a,b,k,'RankFilter', 'evpos',@update_sliders), interface_params, k);

	k=k+1;
	interface_params =  SetSliderParams('Set Negative EV offset', 41, 1, UnsharpMasking.evneg, 1, @(a,b)SetParam(a,b,k,'RankFilter', 'evneg',@update_sliders), interface_params, k);

	k=k+1;
	interface_params =  SetSliderParams('Set Positive ER Offset', 225, 0, UnsharpMasking.erpos, 1, @(a,b)SetParam(a,b,k,'RankFilter', 'erpos',@update_sliders), interface_params, k);

	k=k+1;
	interface_params =  SetSliderParams('Set Negative ER Offset', 225, 0, UnsharpMasking.erneg, 1, @(a,b)SetParam(a,b,k,'RankFilter', 'erneg',@update_sliders), interface_params, k);

	k=k+1;
	interface_params =  SetSliderParams('Set Window Width', 15, 1, UnsharpMasking.WdSzX, 2, @(a,b)SetParam(a,b,k,'RankFilter', 'WdSzX',@update_sliders), interface_params, k);

	k=k+1;
	interface_params =  SetSliderParams('Set Window Height', 15, 1, UnsharpMasking.WdSzY, 2, @(a,b)SetParam(a,b,k,'RankFilter', 'WdSzY',@update_sliders), interface_params, k);



	UnsharpMasking.buttongroup_handle = SetInteractiveInterface(handles, interface_params); 
		             

    imshow(UnsharpMasking.im, [0 255], 'parent', UnsharpMasking.axes_1);
    DisplayAxesTitle( UnsharpMasking.axes_1, 'Test image','TM',10);
    
% 	process_image (UnsharpMasking.im, UnsharpMasking.evpos, UnsharpMasking.evneg, UnsharpMasking.erpos, UnsharpMasking.erneg, ...
% 	UnsharpMasking.WdSzX, UnsharpMasking.WdSzY, ...
% 	UnsharpMasking.axes_1, UnsharpMasking.axes_2, UnsharpMasking.axes_3, UnsharpMasking.axes_4, ...
% 	UnsharpMasking.axes_5, UnsharpMasking.axes_6);
...
UnsharpMasking = handles.RankFilter;
process_image (UnsharpMasking.im, UnsharpMasking.evpos, UnsharpMasking.evneg, UnsharpMasking.erpos, UnsharpMasking.erneg, ...
    UnsharpMasking.WdSzX, UnsharpMasking.WdSzY, ...
    UnsharpMasking.axes_1, UnsharpMasking.axes_2, UnsharpMasking.axes_3, UnsharpMasking.axes_4, ...
    UnsharpMasking.axes_5, UnsharpMasking.axes_6);
...
function process_image (im, evpos, evneg,  ERplus, ERminus, LX, LY,  axes_1, axes_2,  axes_3, axes_4,  axes_5, axes_6)
...
i = i+1; waitbar(i/iteration_num,wait_bar_handle);
 ...
 % std_ev_mb
std_ev_out=std(EVnbh,1, 2)./sizeev;
std_ev_out=reshape(std_ev_out, [SzY SzX]);
i = i+1; waitbar(i/iteration_num,wait_bar_handle);

delete(wait_bar_handle);
imshow(mean_ev_out, [0 255], 'parent', axes_1);
DisplayAxesTitle( axes_1, ['Mean Of EV-Neighborhood'],   'TM'); 

imshow(medn_ev_out, [0 255], 'parent', axes_2);
DisplayAxesTitle( axes_2, ['Median Of EV-Neighborhood'],   'TM'); 


imshow(maxminev, [], 'parent', axes_3);
DisplayAxesTitle( axes_3, ['Max-Min Over EV-Neighborhood'],   'TM'); 


imshow(mx_mn_er, [], 'parent', axes_4);
DisplayAxesTitle( axes_4, ['Max-Min over ER-neighborhood'],   'BM'); 

imshow(size_ev_out, [], 'parent', axes_5);
DisplayAxesTitle( axes_5, ['Size Of EV Neighborhood'],   'BM'); 

imshow(std_ev_out, [], 'parent', axes_6);
DisplayAxesTitle( axes_6, ['STD Over EV Neighborhood'],   'BM'); 
end